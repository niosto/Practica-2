...
data Vehiculo = Vehiculo {
    placa :: String,                -- La placa del vehículo
    entrada :: UTCTime,             -- La hora de entrada del vehículo
    salida :: Maybe UTCTime         -- La hora de salida del vehículo, si ha salido del parqueadero
} deriving (Show, Read, Generic, NFData)  -- Derivamos instancias de clases de tipo necesarias
...
-- Función para convertir una línea del archivo de texto en un vehículo
leerArchivo :: String -> Maybe Vehiculo
leerArchivo linea = case reads linea :: [(Vehiculo, String)] of
    [(v, "")] -> Just v
    _         -> Nothing
...
-- Función para mostrar el historial del parqueadero
leer_vehiculos :: [Vehiculo] -> IO ()
leer_vehiculos = mapM_ (putStrLn . mostrarVehiculo) --Se usa un map maybe, aplicando mostrar vehiculo a cada elemento
...
cargarParqueadero = do
    ...
    let vehiculos = mapMaybe leerArchivo lineas
...
mostrarVehiculo vehiculo =
    "Placa: " ++ placa vehiculo ++ ", Entrada: " ++ show (entrada vehiculo) ++ ", Salida: " ++ show (salida vehiculo)
...
cicloPrincipal parqueadero = do
    ...
    case opcion of
        ...
        "4" -> do
            putStrLn "Mostrando lista de vehiculos del parqueadero:"
            leer_vehiculos parqueadero
            cicloPrincipal parqueadero
